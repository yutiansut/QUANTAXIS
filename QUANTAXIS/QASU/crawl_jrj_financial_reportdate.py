...
from QUANTAXIS_CRAWLY.jrj_simulation_web import *
...
def QA_SU_save_report_calendar_day(client=DATABASE, ui_log = None, ui_progress = None):
    ...
    def __saving_work(report_date, report_calendar):
        try:
            ...
            report_calendar.insert_many(
                get_financial_report_date(report_date,4), ordered=False)
    ...
    for item in range(len(date_list)):
        ...
        strProgressToLog = 'DOWNLOAD PROGRESS {}'.format(str(float(item / len(date_list) * 100))[0:4] + '%', ui_log)
        intProgressToLog = int(float(item / len(date_list) * 100))
        QA_util_log_info(strProgressToLog, ui_log= ui_log, ui_progress= ui_progress, ui_progress_int_value= intProgressToLog)

        __saving_work( date_list[item], report_calendar)
