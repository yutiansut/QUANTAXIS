...
if __name__ == '__main__':
    ...
    hma_croos_day = data_day.add_func(hma_cross_func)
    ...
    code = codelist[0]
    actions = (hma_croos_day.loc[(slice(None), code), :]['BOOTSRTAP_R5'].diff() != 0)
    actions = actions[actions.apply(lambda x: x == True)]  # eqv.  Trim(x == False)
    #print(actions)
    orders = hma_croos_day.loc[((actions == True).index.get_level_values(level=0), code), :]
    print(orders)
    ...
    plt.plot(DATETIME_LABEL, hma_croos_day.loc[(slice(None), code), 'HMA10'].values, 'c', linewidth = 0.6, alpha = 0.75)
    actions = (hma_croos_day.loc[(slice(None), code), :]['BOOTSRTAP_R5'].diff() != 0)
    ax1.plot(DATETIME_LABEL, 
             np.where((actions.values == True) & (hma_croos_day.loc[(slice(None), code), 'BOOTSRTAP_R5'].values > 0), 
