...
def QSDD(dataframe,SHORT=12,LONG=26,M=9):
    ...
    A = QA.MA(-100 * (QA.HHV(HIGH, 34) - CLOSE) / (QA.HHV(HIGH, 34) - QA.LLV(LOW, 34)), 19)
    B = -100 * (QA.HHV(HIGH, 14) - CLOSE) / (QA.HHV(HIGH, 14) - QA.LLV(LOW, 14))
    D = QA.EMA(-100 * (QA.HHV(HIGH, 34) - CLOSE) / (QA.HHV(HIGH, 34) - QA.LLV(LOW, 34)), 4)
