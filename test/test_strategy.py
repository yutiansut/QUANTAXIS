...
class backtest(QA.QA_Backtest):
    def init(self):
        ...
        self.strategy_start_date='2010-01-10'
        self.strategy_end_date='2015-01-28'
        self.strategy_stock_list=['000001.SZ']
    ...
    def handle_data(self):
        ...
        for i in range(int(self.start_mes['id']),int(self.end_mes['id']),1):
            ...
            result=example_strategy(data['market'],data['account']['body']['account']['profit']*100,data['account']['body']['account']['hold'],data['account']['body']['account']['cur_profit_present']*100)
            
            if int(result)==1 and int(data['account']['body']['account']['hold'])==0:
                ...
                #print(message)
            elif int(result)==1 and int(data['account']['body']['account']['hold'])==1:
            ...
            elif int(result)==0 and int(data['account']['body']['account']['hold'])==0:
                 QA.QA_util_log_info('ZERO and Watch!!!!!!!!!!!!')
            elif int(result)==0 and int(data['account']['body']['account']['hold'])==1:
                self.bid.bid['amount']=int(data['account']['body']['account']['portfolio']['amount'])
                self.bid.bid['price']=float(data['market'][-1][4])
                self.bid.bid['code']=str(self.strategy_stock_list[0])[0:6]
                self.bid.bid['time']=data['market'][-1][6]
...
def example_strategy(data,profit,hold,curprofit):
    if hold==0:
        return 1
    else: return 0
...
stock_list=pymongo.MongoClient().quantaxis.stock_list
stoocks=stock_list.find_one()['stock']['code']
for item in stoocks:
